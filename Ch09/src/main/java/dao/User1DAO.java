...
public class User1DAO extends DBHelper{
	...
	public User1VO selectUser1(String uid) {
		...
		try {
			conn = getConnection();
			conn.prepareStatement("select * from `user1` where `uid`=?");
	...
	public void updateUser1(User1VO vo) {
		try {
			conn = getConnection();
			conn.prepareStatement("update `user1` set `name`=?, `hp`=?, `age`=? where `uid`=?");
	...
	public void deleteUser1(String uid) {
		try {
			conn = getConnection();
			conn.prepareStatement("delete from `user1` where `uid`=?");
			psmt.setString(1, uid);
			psmt.executeUpdate();
			close();
		}catch (Exception e) {
